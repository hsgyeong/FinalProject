...
<script type="text/javascript">
    ...
    //         var tag = "<tr><th class='num'>번호</th><th class='room'>고객 ID</th><th class='go'></th></tr>"; // 'tag' 변수는 HTML 문자열로 시작하며, 채팅방 목록을 나타내는 테이블의 첫 번째 행을 정의합니다.
    ...
    <div id="roomContainer" class="roomContainer">
        <table id="roomList" class="roomList">
            ...
            <c:if test="${sessionScope.admin_id!=null}">
                <c:forEach var="res" item="${}" index="idx">
                    <tr>
                        <td class="num">${idx + 1}</td>
                        <td class="room">${res.roomName}</td>
                        <td class="go"><button type="button" onclick="goRoom('${res.roomNumber}', '${res.roomName}')">참여</button></td>
                    </tr>
                </c:forEach>
            </c:if>
